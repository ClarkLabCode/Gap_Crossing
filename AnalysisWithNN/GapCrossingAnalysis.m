...
ZapStationaryFlipsHan = @ZapStationaryFlips;
FindCrossEventsHan = @FindCrossEvents;
...
dlgList = {'Genotype','Directory Name','Input File Name','Experiment Repetition Number',...
...
ExpRepNum           = str2double(expParams{4});
flipRate            = str2double(expParams{5});
expLength           = str2double(expParams{6});
temp                = str2double(expParams{7});
dateAcq             = expParams{8};
timeAcq             = expParams{9};
eclosionDate        = expParams{10};
timeZone            = expParams{11};
notes               = expParams{12};
CassetteID          = str2double(expParams{13});
NumCorridors        = str2double(expParams{14});
NumGaps             = str2double(expParams{15});
sizeThreshCutOff    = str2double(expParams{16});
indPosFrameBuffer   = str2double(expParams{17});
erodePix            = str2double(expParams{18});
...
mkdir(['Experiment_', num2str(ExpRepNum)]);

% Navigate into new folder
cd(['Experiment_', num2str(ExpRepNum)]);
...
% Feed the FlipBinnedFlyStruct into the appropriate structure based on
% which number repetition of the experiment the data came from
FlipBinnedFlyStruct(ExpRepNum) = FlipBinnerHan(finalFlyStruct);
FlipBinnedFlyStruct(ExpRepNum) = ZapStationaryFlipsHan(FlipBinnedFlyStruct);
FlipBinnedFlyStruct = FindCrossEventsHan(FlipBinnedFlyStruct, ExpRepNum, NumGaps);
...
cd ../Fly_Structure
save([directoryName,'_FlipBinnedFlyStruct_Exp',num2str(ExpRepNum),'.mat'],'FlipBinnedFlyStruct');
save([directoryName,'_Exp',num2str(ExpRepNum),'_analyzed_workspace.mat']);
