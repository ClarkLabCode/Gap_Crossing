...
for i_omega = 1:length(omega_sigmaVec)
    ...
    % xBinLims = 10.^[-orderMagBelow1:0.1:-0.1,0:0.05:orderMagAbove1x]; % Log-log bins
    % yBinLims = 10.^[-orderMagBelow1:0.1:-0.1,0:0.05:orderMagAbove1y]; % Log-log bins
    xBinLims = 10.^[-orderMagBelow1:0.1:orderMagAbove1x]; % Log-log bins
    yBinLims = 10.^[-orderMagBelow1:0.1:orderMagAbove1y]; % Log-log bins
    % xBinLims = 10^(-orderMagBelow1):10^(-orderMagBelow1):10^(orderMagAbove1x); % Linear bins
    % yBinLims = 10^(-orderMagBelow1):10^(-orderMagBelow1):10^(orderMagAbove1y); % Linear bins
    ...
    for i_D1overRc = 1:(length(xBinLims)-1)
        for i_D2overRc = 1:(length(yBinLims)-1)
            ...
            if mod(progCounter,100) == 0
                disp([num2str(progCounter/length(xBinLims)/length(yBinLims)*100),'% complete']);
            end
            ...
            [ratio_fit,temp_mdl_ratio_to_ratio] = fit(tempD1./tempD2,tempPhiDot2./tempPhiDot1,'poly1');
            ...
            if (xBinLims(i_D1overRc+1) == 0.1) && (yBinLims(i_D2overRc) == 0.1)
                ...
                plot(ratio_fit,'k--')
                xlabel('$D1/D2$','Interpreter','latex')
                ylabel('$\dot{\phi_1}/\dot{\phi_2}$','Interpreter','latex')
                % xlabel('$\frac{D1}{D2}$','Interpreter','latex')
                % ylabel('$\frac{\dot{\phi_1}}{\dot{\phi_2}}$','Interpreter','latex')
            elseif (xBinLims(i_D1overRc+1) == 1) && (yBinLims(i_D2overRc+1) == 10)
                figure
                hold on
                scatter(tempD1./tempD2,tempPhiDot2./tempPhiDot1,10,[0.7,0.7,0.7],'filled')
                plot(ratio_fit,'k--')
                xlabel('$D1/D2$','Interpreter','latex')
                ylabel('$\dot{\phi_1}/\dot{\phi_2}$','Interpreter','latex')
                disp(num2str(tempRatioToRatioRSquared))
                % ylim([-3000,1])
                % xlabel('$\frac{D1}{D2}$','Interpreter','latex')
                % ylabel('$\frac{\dot{\phi_1}}{\dot{\phi_2}}$','Interpreter','latex')
            end
    ...
    sizeOfBinsInLogSpace((yBinLims >= 10^orderMagAbove1y),:) = 0;
    sizeOfBinsInLogSpace(:,(xBinLims >= 10^orderMagAbove1x)) = 0;
        ...
        for i_y = 1:(length(yBinLims)-1)
            ...
            if i_x == i_y
                patch([xBinLims(i_x),xBinLims(i_x),xBinLims(i_x+1),xBinLims(i_x+1)],...
                      ...
                      occurrenceFreqDensForPlot(i_y+1,i_x),... % 
                      'EdgeColor','none')
    ...
    tempOccurrenceFreqDensForPlot((yBinLims >= 10^orderMagAbove1y),:) = 0;
    tempOccurrenceFreqDensForPlot(:,(xBinLims >= 10^orderMagAbove1x)) = 0;
    ...
    dashLineVec = [10^(-orderMagBelow1):10^(max(orderMagAbove1x,orderMagAbove1y))];
    ...
    set(gca, 'YScale', 'log')
    set(gca, 'XScale', 'log')
    xticks(10.^[-orderMagBelow1:orderMagAbove1x])
    xticklabels(10.^[-orderMagBelow1:orderMagAbove1x])
    yticks(10.^[-orderMagBelow1:orderMagAbove1y])
    yticklabels(10.^[-orderMagBelow1:orderMagAbove1y])
    pbaspect([1,1,1])
    xlim([10^(-orderMagBelow1),10^(orderMagAbove1x)])
    ylim([10^(-orderMagBelow1),10^(orderMagAbove1y)])
    xlabel('D_1/R_C')
    ylabel('D_2/R_C')
    ...
    text(10^(2/5),10^(orderMagAbove1y)*10^(-1/8),'Reg Bar / Reg BG','HorizontalAlignment','center','FontSize',18,'Color','w');

    % Next we plot the histogram for phiDot2/phiDot1 vs D1/D2 and print the
    % title of what the plot is showing
    makePlotForPhiDotSims(xBinLims,yBinLims,ratioToRatioRSquared,orderMagBelow1,orderMagAbove1x,orderMagAbove1y,maxD1overRc,maxD2overRc,sizeOfBinsInLogSpace,phi1and2atCenterOfRF,v_x_mean,phi_const,omega_sigma,max_D1,max_D2_from_D1,normOccurrenceFreqDensTo1,occurrenceFreqDensForPlot);
    ...
    if computeOnlyRatioTerms == 0
        ...
        % title of what the plot is showing
        makePlotForPhiDotSims(xBinLims,yBinLims,phiDot1ToRatioRSquared,orderMagBelow1,orderMagAbove1x,orderMagAbove1y,maxD1overRc,maxD2overRc,sizeOfBinsInLogSpace,phi1and2atCenterOfRF,v_x_mean,phi_const,omega_sigma,max_D1,max_D2_from_D1,normOccurrenceFreqDensTo1,occurrenceFreqDensForPlot);
        ...
        % title of what the plot is showing
        makePlotForPhiDotSims(xBinLims,yBinLims,phiDot2ToRatioRSquared,orderMagBelow1,orderMagAbove1x,orderMagAbove1y,maxD1overRc,maxD2overRc,sizeOfBinsInLogSpace,phi1and2atCenterOfRF,v_x_mean,phi_const,omega_sigma,max_D1,max_D2_from_D1,normOccurrenceFreqDensTo1,occurrenceFreqDensForPlot);
        ...
        % title of what the plot is showing
        makePlotForPhiDotSims(xBinLims,yBinLims,ratioToD1RSquared,orderMagBelow1,orderMagAbove1x,orderMagAbove1y,maxD1overRc,maxD2overRc,sizeOfBinsInLogSpace,phi1and2atCenterOfRF,v_x_mean,phi_const,omega_sigma,max_D1,max_D2_from_D1,normOccurrenceFreqDensTo1,occurrenceFreqDensForPlot);
        ...
        % title of what the plot is showing
        makePlotForPhiDotSims(xBinLims,yBinLims,phiDot1ToD1RSquared,orderMagBelow1,orderMagAbove1x,orderMagAbove1y,maxD1overRc,maxD2overRc,sizeOfBinsInLogSpace,phi1and2atCenterOfRF,v_x_mean,phi_const,omega_sigma,max_D1,max_D2_from_D1,normOccurrenceFreqDensTo1,occurrenceFreqDensForPlot);
        ...
        % title of what the plot is showing
        makePlotForPhiDotSims(xBinLims,yBinLims,phiDot2ToD1RSquared,orderMagBelow1,orderMagAbove1x,orderMagAbove1y,maxD1overRc,maxD2overRc,sizeOfBinsInLogSpace,phi1and2atCenterOfRF,v_x_mean,phi_const,omega_sigma,max_D1,max_D2_from_D1,normOccurrenceFreqDensTo1,occurrenceFreqDensForPlot);
...
function makePlotForPhiDotSims(xBinLims,yBinLims,RSquaredUsed,orderMagBelow1,orderMagAbove1x,orderMagAbove1y,maxD1overRc,maxD2overRc,sizeOfBinsInLogSpace,phi1and2atCenterOfRF,v_x_mean,phi_const,omega_sigma,max_D1,max_D2_from_D1,normOccurrenceFreqDensTo1,occurrenceFreqDensForPlot)
...
% Open a new full screen figure window
    ...
    if fracInfoCapturedRegReg < 0.01
        text(10^(2/5),10^(orderMagAbove1y)*10^(-1/4),[num2str(fracInfoCapturedRegReg,'%.2e')],'HorizontalAlignment','center','FontSize',18,'Color','w');
    else
        text(10^(2/5),10^(orderMagAbove1y)*10^(-1/4),[num2str(fracInfoCapturedRegReg,3)],'HorizontalAlignment','center','FontSize',18,'Color','w');
    end
