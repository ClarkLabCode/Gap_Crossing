...
for i_omega = 1:length(omega_sigmaVec)
    ...
    % omega = sign(phi_const)*abs(normrnd(0,omega_sigma,numSamples,1)); % Pull omega from a half-Gaussian
    omega = normrnd(0,omega_sigma,numSamples,1); % Pull omega from a Gaussian
    ...
    % xBinLims = 10.^[-orderMagBelow1:0.25:-0.25,0:0.5:orderMagAbove1]; % Log-log bins
    % yBinLims = 10.^[-orderMagBelow1:0.25:-0.25,0:0.5:orderMagAbove1]; % Log-log bins
    xBinLims = 10.^[-orderMagBelow1:0.05:orderMagAbove1]; % Log-log bins
    yBinLims = 10.^[-orderMagBelow1:0.05:orderMagAbove1]; % Log-log bins
    ...
    for i_D1overRc = 1:(length(xBinLims)-1)
        for i_D2overRc = 1:(length(yBinLims)-1)
            ...
            % Produce the panels for fig 6 supp R^2
            if (xBinLims(i_D1overRc) == 10^(-0.75)) && (yBinLims(i_D2overRc) == 10^(-0.5))
                figure
                hold on
                ...
                disp(['Bottom left = (', num2str(xBinLims(i_D1overRc)), ', ', num2str(yBinLims(i_D2overRc)),')'])
            ...
            elseif (xBinLims(i_D1overRc) == 10^(-0.25)) && (yBinLims(i_D2overRc) == 10^(0.75))
                figure
                hold on
                scatter(tempD1./tempD2,tempPhiDot2./tempPhiDot1,10,[0.7,0.7,0.7],'filled')
                plot(ratio_fit,'k--')
    ...
    % Prog / Prog text
    text(10^(-orderMagBelow1/2),10^(-1/4),[num2str(sum(occurrenceFreqDensForPlot(yBinLims < 1, xBinLims < 1),'all'),3)],'HorizontalAlignment','center','FontSize',18,'Color','w');
    
    % Prog / Reg text
    text(10^(-orderMagBelow1/2),10^(1/8),[num2str(sum(occurrenceFreqDensForPlot(yBinLims >= 1, xBinLims < 1),'all'),3)],'HorizontalAlignment','center','FontSize',18,'Color','w');

    % Reg / Reg text
    text(10^(2/5),10^(orderMagAbove1)*10^(-1/4),[num2str(sum(occurrenceFreqDensForPlot(yBinLims >= 1, xBinLims >= 1),'all'),3)],'HorizontalAlignment','center','FontSize',18,'Color','w');
...
% Open a new full screen figure window
    ...
    for i_x = 1:(length(xBinLims)-1)
        for i_y = 1:(length(yBinLims)-1)
            ...
            if i_x == i_y
                patch([xBinLims(i_x),xBinLims(i_x),xBinLims(i_x+1),xBinLims(i_x+1)],...
                      [xBinLims(i_x),yBinLims(i_y+1),yBinLims(i_y+1),xBinLims(i_x+1)],...
                      [RSquaredUsed(i_y,i_x);RSquaredUsed(i_y+1,i_x);RSquaredUsed(i_y+1,i_x+1);RSquaredUsed(i_y+1,i_x+1)],... % 
                      'EdgeColor','none')
            % Away from the diagonals, the patches should be rectangles
            else
                patch([xBinLims(i_x),xBinLims(i_x),xBinLims(i_x+1),xBinLims(i_x+1)],...
                      [yBinLims(i_y),yBinLims(i_y+1),yBinLims(i_y+1),yBinLims(i_y)],...
                      [RSquaredUsed(i_y,i_x);RSquaredUsed(i_y+1,i_x);RSquaredUsed(i_y+1,i_x+1);RSquaredUsed(i_y,i_x+1)],... % 
                      'EdgeColor','none')
            end
        end
    end
    ...
    % size of the bins in log space as the relative weights
        ...
        fracInfoCapturedProgProg = sum(RSquaredUsed(yBinLims < 1, xBinLims < 1),'all')/sum(tempSizeMat~=0,'all');
        tempSizeMat = sizeOfBinsInLogSpace(yBinLims >= 1, xBinLims < 1);
        fracInfoCapturedProgReg = sum((tempSizeMat/sum(tempSizeMat,'all')).*RSquaredUsed(yBinLims >= 1, xBinLims < 1),'all');
        fracInfoCapturedProgReg = sum(RSquaredUsed(yBinLims >= 1, xBinLims < 1),'all')/sum(tempSizeMat~=0,'all');
        tempSizeMat = sizeOfBinsInLogSpace(yBinLims >= 1, xBinLims >= 1);
        fracInfoCapturedRegReg = sum((tempSizeMat/sum(tempSizeMat,'all')).*RSquaredUsed(yBinLims >= 1, xBinLims >= 1),'all');
        fracInfoCapturedRegReg = sum(RSquaredUsed(yBinLims >= 1, xBinLims >= 1),'all')/sum(tempSizeMat~=0,'all');
    end
