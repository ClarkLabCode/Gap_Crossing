function [ Qpre, p] = fit_logistic_Qinffixed(t,Q)
...
%   thalf, Qinf, and alpha. Qinf is fixed for this function
...
%
%   Example:
%       Qinf = 1; alpha = 0.33; thalf = 108.5;
%       t = 100:120;
%       Q = Qinf./(1 + exp(-alpha*(t-thalf)));
%       noise = randn(1,length(t));
%       Qd = Q+noise;
%       Qpre = fit_logistic_Qinffixed(t,Qd);
%       figure(1)
%       clf
%       hold on
%       plot(t,Qd,'o')  % data
...
%       and start over.
%   May 5, 2015: Modified for case of fixed value of Qinf
...
G = zeros(length(t),2) ;    % dimensionalize partial derivative matrix
...
while epsilon > thresh
  ...
  %G(:,2) = 1./(1 + ee) ;                % dd/dQinf
  G(:,2) = Qinf*(t-thalf).*(ee.*eee) ;  % dd/dalpha
