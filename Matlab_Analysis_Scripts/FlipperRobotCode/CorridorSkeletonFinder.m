...
for oddEven = 1:2
    ...
    checkForErr = questdlg('Does the skeleton look correct?', 'Skeleton Check', 'Yes', 'No', 'Yes');
    
    % If an error happened, cancel out of the function and inform user
    % which file the error occurred for
    if ~strcmp(checkForErr,'Yes')
        error(['User indicated an error in file: ', inputFileName]);
    end
