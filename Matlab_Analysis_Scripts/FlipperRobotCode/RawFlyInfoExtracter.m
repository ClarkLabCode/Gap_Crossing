...
function WS = RawFlyInfoExtracter(WS)
...
numFlips = (length(indPos)/2) - 1; % How many flips take place in a video
...
for flipCounter = 1:numFlips
    ...
    imwrite(uint8(median(flipArray,3)), bg_img_name);
    
    % Bg subtract flipArray, then threshold
    flipArray = median(flipArray,3) - flipArray;
    ...
    if flipCounter == 1
        for frameCountInFlip = 1:sizeOfTemp(4)
            ...
            if frameCountInFlip == 1
                ...
                % Make sure that stats isn't empty
                if length(stats)
    ...
    if mod(flipCounter,10) == 0
        disp(['Video processed through flip ', num2str(flipCounter), ' for file: ', inputFileName])
    end
end
