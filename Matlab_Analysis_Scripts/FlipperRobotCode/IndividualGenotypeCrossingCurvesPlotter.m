...
switch genotypesCompared
    ...
    case 'Other comparison(s)'
        % Ask the user if they also want a synthetic control or 4th curve
        fourthCurve = ...
            questdlg('Do you want a fourth curve?','Fourth curve on plot',...
                     'Yes, synthetic control','Yes, other curve',...
                     'No','Yes, synthetic control');

        % Ask the user to select the desired curves to compare
        Gal4UASIdx = input('Select the red curve (typically Gal4 > UAS) by typing the ordered pair of that genotype in AllCrossStatsNames.\n');
        EmptyUASIdx = input('Select the dark gray curve (typically sp empty (DBD) > UAS) by typing the ordered pair of that genotype in AllCrossStatsNames.\n');
        Gal4PlusIdx = input('Select the light gray curve (typically Gal4 > +) by typing the ordered pair of that genotype in AllCrossStatsNames.\n');
        % If the user chose to include a synthetic control curve
        if strcmp(fourthCurve,'Yes, synthetic control')
        ...
        % If the user chose to include a fourth curve that isn't the synthetic control
        elseif strcmp(fourthCurve,'Yes, other curve')
            IsoD1Idx = input('Select the black curve (typically IsoD1) by typing the ordered pair of that genotype in AllCrossStatsNames.\n');
        end
        ...
        shadedErrorBar(1:0.5:2.5,...
             ...
             'lineprops', {'Color',[0.3,0.3,0.3],'Marker','none'});
        % If the user chose to include a synthetic control curve, plot it
        if strcmp(fourthCurve,'Yes, synthetic control')
            % Synthetic control (black)
            shadedErrorBar(1:0.5:2.5,...
                 synth_mean,...
            ...
            legend({AllCrossStatsNames{Gal4UASIdx(1),Gal4UASIdx(2)},...
                    ...
                    'Synthetic control'})
            hold off
        % If the user chose to include a fourth curve that isn't the synthetic control, plot it
        elseif strcmp(fourthCurve,'Yes, other curve')
            % Curve 4 (usually WT control) (black)
            shadedErrorBar(1:0.5:2.5,...
                 mean(AllCrossStats{IsoD1Idx(1),IsoD1Idx(2)}.AllUpVecProperCrossOverAllGapEventRate),...
                 AllCrossStats{IsoD1Idx(1),IsoD1Idx(2)}.AllUpVecProperCrossOverAllGapEventStd,...
                 'lineprops', {'Color','k','Marker','none'});
            % Populate the legend using the names of the experiments used
            legend({AllCrossStatsNames{Gal4UASIdx(1),Gal4UASIdx(2)},...
                    AllCrossStatsNames{EmptyUASIdx(1),EmptyUASIdx(2)},...
                    AllCrossStatsNames{Gal4PlusIdx(1),Gal4PlusIdx(2)},...
                    AllCrossStatsNames{IsoD1Idx(1),IsoD1Idx(2)}})
            hold off
        % If there were only 3 curves included, make the appropriate legend
        else
            % Populate the legend using the names of the experiments used
            legend({AllCrossStatsNames{Gal4UASIdx(1),Gal4UASIdx(2)},...
                    AllCrossStatsNames{EmptyUASIdx(1),EmptyUASIdx(2)},...
