...
%  
%  Meant to be used with the output of CombineExpStructs and TrackHead and 
%  then used as the input for AnalyzeCrossStats
...
%             WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedFoldedVelTheta = 0;
        ...
        % the units into mm and sec (previously pixels and frames).
            ...
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedHeadX = ...
                ((WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).HeadX - CorridorCenterXOdd)*cosd(CorridorAngleOdd) - ...
                 (WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).HeadY - CorridorCenterYOdd)*sind(CorridorAngleOdd))/PixelsPerMM;
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedHeadY = -1*...
                ((WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).HeadY - CorridorCenterYOdd)*cosd(CorridorAngleOdd) + ...
                 (WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).HeadX - CorridorCenterXOdd)*sind(CorridorAngleOdd))/PixelsPerMM;
            ...
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedFoldedHeadX = ...
                abs(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedHeadX);
            ...
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedFoldedVelY = ...
                ...
                diff(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedTimeInFlip);
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedFoldedVelHeadX = ...
                diff(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedFoldedHeadX) ./ ...
                diff(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedTimeInFlip(1:end-1));
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedFoldedVelHeadY = ...
                diff(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedHeadY) ./ ...
                diff(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.OddFlips(oddFlipCounter).AlignedTimeInFlip(1:end-1));
...
%             WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedFoldedVelTheta = 0;
        ...
        % the units into mm and sec (previously pixels and frames).
            ...
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedHeadX = ...
                ((WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).HeadX - CorridorCenterXEven)*cosd(CorridorAngleEven) - ...
                 (WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).HeadY - CorridorCenterYEven)*sind(CorridorAngleEven))/PixelsPerMM;
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedHeadY = -1*...
                ((WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).HeadY - CorridorCenterYEven)*cosd(CorridorAngleEven) + ...
                 (WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).HeadX - CorridorCenterXEven)*sind(CorridorAngleEven))/PixelsPerMM;
            % To properly transform orientation of the ellipse, we rotate 
            % it by CorridorAngle and need to keep it bound from -90 to 90
            % degrees (which is the reason for the nested mods)
            % Also note the -1 corresponding to flipping the y axis in the
            ...
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedFoldedHeadX = ...
                abs(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedHeadX);
            ...
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedFoldedVelY = ...=
                ...
                diff(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedTimeInFlip);
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedFoldedVelHeadX = ...
                diff(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedFoldedHeadX) ./ ...
                diff(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedTimeInFlip(1:end-1));
            WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedFoldedVelHeadY = ...=
                diff(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedHeadY) ./ ...
                diff(WS.FlipBinnedFlyStruct(flyCounter).AlignedData.EvenFlips(evenFlipCounter).AlignedTimeInFlip(1:end-1));
            % Doing this for theta is a little complicated because of the
            % periodicity of theta, but luckily we can use the unwrap
            % function in matlab to do this easily. Because we have an
