...
% % Make sure we navigate to the folder this script expects us to be in
% cd 'C:\Users\clarklab\Joe\Gap_Crossing\Matlab_Analysis_Scripts\FlipperRobotCode'
% 
% % Go ahead and add this to path in case it's not there for some reason
...
if strcmp(prevAnalysis,'Yes')
    WS_names = LoadGCExpWSNamesFunc(NumGCExp);
...
% user to fill in the necessary info
    ...
    WS_all = RequestUserGCInputsFunc(NumGCExp, StandardAnalysis);
    
    % Call function to set up the directories for all the experiments
    WS_names = SetUpGCExpDirectoriesFunc(WS_all);
...
tic
    ...
    switch AnalysisStep
        case ProcessVideo
            % Since this is a time-consuming step, we are better off using
            % parallel computing for this step if we have 3+ experiments.
            % Note that it is important to only use parallel computing when we
            % have 3+ experiments loaded in because there is substantial
            % overhead associated with parallel computing that is not overcome
            % unless 3+ cores are in fact being used.
            if NumGCExp > 20
                % Since this step stores very little in our RAM, we can get
                % away with using all of our available CPU cores minus one
                % because we aren't afraid of RAM overload errors, so we
                % initialize the parallel pool as all but one CPU cores.
                % It's also important not to assign more cores to the job than
                % we have experiments because this adds further overhead time
                % for the parallel computing, so choose the minimum # of cores.
                parpool(min((maxNumCompThreads - 1), NumGCExp));

                % Run a parallel for loop across experiments and make use of
                % the necessary parallel function equivalents for saving
                parfor vidCounter = 1:NumGCExp
                    ...
                    if SkeletonMade
                        ...
                        parsaveWSFunc([WS.directoryName,'_Exp',num2str(WS.ExpRepNum),'_WS_struct.mat'],WS);
                ...
                % Make use of a normal for loop to avoid the unnecessary
                % overhead of parallel computing for so few experiments
        ...
        case MakeSkeleton
            % Loop through all experiments loaded in, and do this without
            % parallel computing because we are requesting user inputs
            for vidCounter = 1:NumGCExp
        ...
        case RunNeuralNet
            ...
            if NumGCExp > 20
                ...
                % Since this step stores so much in our RAM, we can run only 3
                % of our cores in parallel without overloading our RAM, so we
                % initialize the parallel pool as exactly 3 cores. This
                % requires at least 64 GB of RAM to be installed in the PC.
                parpool(3);
        ...
        case RunFinalStep
            % Loop through all experiments loaded in, and do this without
            % parallel computing because the overhead associated with parallel
            % computing is too high for such a fast task
            for vidCounter = 1:NumGCExp
                % Load in WS for each experiment
                WS = load(WS_names{vidCounter});
                % To get the correct WS, we must go in one layer
